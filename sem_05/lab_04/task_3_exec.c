...
int main(void)
{
    const char *executables[CHILDS_COUNT] = {"./lewenstein.exe", "./stack.exe"};
    const char *files[CHILDS_COUNT] = {"lewenstein.exe", "stack.exe"};
    ...
    for (size_t i = 0; i < CHILDS_COUNT; i++)
    {
        ...
        else
        {
            ...
            int rc = execlp(executables[i], files[i], NULL);
            if (rc == -1)
            {
                printf("Child №%zu exec error\n", i);
